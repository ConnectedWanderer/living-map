export const MOCK_FEED_URL: string = process.env.MOCK_FEED_URL || 'http://localhost:3001';

export const LE_URL: string = process.env.LE_URL || 'http://localhost:8000';
...
export async function ensureServices(): Promise<void> {
  ...
  if (failures.length > 0) {
    throw new Error(`Required services not healthy: ${failures.join(', ')}`);
  }
}
