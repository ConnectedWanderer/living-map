...
describe("runSource", () => {
  it("fetches articles, inserts, enriches each, updates each, logs summary", async () => {
    const articles = [
      { source_id: "a1", title: "Article A", description: "Desc A", url: "http://a", published_at: "2026-01-01T00:00:00.000Z", source: "mock-feed" },
      { source_id: "a2", title: "Article B", description: "Desc B", url: "http://b", published_at: "2026-01-02T00:00:00.000Z", source: "mock-feed" },
    ];
    ...
    const deps = {
      ...
      updateLocation: async () => { updateCalls++; },
      extractLocation: async () => { extractCalls++; return geoJson; },
      locationExtractionUrl: "http://le:8000",
      logger: { info: (obj: unknown) => { logInfoCalls.push(obj); }, error: () => {}, warn: () => {} } as unknown as import("pino").Logger,
    };
  ...
  it("skips enrichment when all articles are duplicates", async () => {
    const articles = [
      { source_id: "a1", title: "Article A", description: undefined, url: "http://a", published_at: "2026-01-01T00:00:00.000Z", source: "mock-feed" },
    ];
    ...
    const deps = {
      ...
      updateLocation: async () => { updateCalls++; },
      extractLocation: async () => { extractCalls++; return null; },
      locationExtractionUrl: "http://le:8000",
      logger: { info: (obj: unknown) => { logInfoCalls.push(obj); }, error: () => {}, warn: () => {} } as unknown as import("pino").Logger,
    };
